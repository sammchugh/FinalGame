...
public class Game {
    ...
    public static HashMap<String, Room> roomObjects = new HashMap<>();
	...
	public static void processCommand(String command) {
			...
			case "take":
				if(currentRoom.getItem(words[1])== null) {
					...
					Game.print("You took the "+ words[1]);
				}
				break;
			case "i":
				if(inventory.size()==0) {
			...
			case "light":
                ...
                Game.print("You are trying to light the " + words[1] + ".");
                
                if(currentRoom.getItem(words[1]) != null){
                    currentRoom.getItem(words[1]).light();
                    Game.print("You lit the "+ words[1]);
                }
                    ...
                    if (getItemInventory(words[1]) == null){
                        ...
                        getItemInventory(words[1]).light();
                        Game.print("\n");
                    }
                }
            break;
			...
			case "escape":
				...
				if(currentRoom.getRoomName() == "Basement") {
					if(getItemInventory("toy") != null && getItemInventory("fire") != null && getItemInventory("cipher") != null) {
			...
			case "save":
				if (words.length < 2) {
			        Game.print("Please specify a file name to save the game.");
			    } else {
			        String saveFile = words[1];
			        Game.print("Saving the game to file: " + saveFile);
			        saveGame(saveFile); // Call the saveGame method with the specified file name
			    }
				break;
				
			case "load":
				if (words.length < 2) {
			        Game.print("Please specify a file name to load the game.");
			    } else {
			        String loadFile = words[1];
			        Game.print("Loading the game from file: " + loadFile);
			        loadGame(loadFile);
			        Game.print("Game loaded successfully.");
			    }
			    break;
